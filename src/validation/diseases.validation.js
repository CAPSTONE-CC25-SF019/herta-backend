...
export default {
  ...
  update: Joi.object({
    image: Joi.string().uri().min(5).required().description('Image URL representing the disease').messages({
    ...
    description: Joi.string().min(1).required().description('Updated description of the disease').messages({
      'string.min': 'the description disease must be greater than one character',
  ...
  filterByNames: Joi.object({
    names: Joi.alternatives()
      ...
      .description('Filter diseases by name(s), accepting either a single name or an array of names')
      .messages({
  ...
  filterById: Joi.object({
    id: Joi.string().uuid().required().description('The unique UUID identifier of the disease').messages({
